        ...
        <PageLayout>
            ...
            <ContentContainer>
                <div className="flex flex-col gap-8">
                    <p>LEAPS (Leadership, Exploration and Progress) is an ASEAN-focused industry immersion program, designed to equipand groom students to become industry-relevant future ASEAN-savvy talents.
                        <br /><br />
                        ...
                        <br /><br />
                        Any tertiary student / institute of higher learning from all majors who was a citizen of the ASEAN member countries (includes Brunei, Cambodia, Laos, Malaysia, Myanmar, Philippines, Singapore, Thailand, Indonesia, and Vietnam) are invited to the challenge!</p>
                    <Heading2 className="capitalize">Our Values</Heading2>
                    ...
                    <div className="grid grid-cols-1 gap-x-6 gap-y-2 md:grid-cols-2 md:gap-y-6 lg:grid-cols-3 lg:gap-y-8">
                        {abyaleapsData?.map((blog) => (<BlogCard key={blog.id} date={blog.createdAt} title={blog.title} href={blog.title} image={blog.image ?? ""} imageAlt={blog.title} />))}
                    </div>
                </div>
            </ContentContainer>
        </PageLayout >

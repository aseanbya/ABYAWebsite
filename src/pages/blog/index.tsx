        ...
        <PageLayout>
            ...
            <ContentContainer>
                <div className="mb-8 flex flex-col gap-8">
                    ...
                    <div className="grid grid-cols-1 gap-x-6 gap-y-2 md:grid-cols-2 md:gap-y-6 lg:grid-cols-3 lg:gap-y-8">
                        {prannouncementData?.map((blog) => (<BlogCard key={blog.id} date={blog.createdAt} title={blog.title} image={blog.image ?? ""} href={blog.title} imageAlt={blog.title} />))}
                    </div>
                    <div className="flex w-full justify-center pb-6">
                    ...
                    <div className="grid grid-cols-1 gap-x-6 gap-y-2 md:grid-cols-2 md:gap-y-6 lg:grid-cols-3 lg:gap-y-8">
                        {futureinaseanData?.map((blog) => (<BlogCard key={blog.id} date={blog.createdAt} title={blog.title} image={blog.image ?? ""} href={blog.title} imageAlt={blog.title} />))}
                    </div>
